...
import { X, AlertTriangle, CheckCircle2, Loader2, Power } from 'lucide-react';
...
interface ProductDeactivateModalProps {
  ...
  onConfirm: (product: Product, newStatus: ProductStatus) => Promise<void>;
}
...
export const ProductDeactivateModal: React.FC<ProductDeactivateModalProps> = ({
  ...
  const targetStatus: ProductStatus = isCurrentlyActive ? 'INACTIVE' : 'ACTIVE';

  const handleConfirm = async () => {
    setLoading(true);
    setApiError(null);
    try {
      await onConfirm(product, targetStatus);
      onClose();
    } catch (err: any) {
      setApiError(err?.message || 'Failed to update product status. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="mt-2 space-y-2.5">
          <h3 className="text-base font-bold text-gray-900">
            {isCurrentlyActive ? 'Deactivate Product?' : 'Reactivate Product?'}
          </h3>
          <p className="text-xs text-gray-600 leading-relaxed">
            {isCurrentlyActive ? (
              <>
                Are you sure you want to deactivate{' '}
                <strong className="text-gray-900 font-semibold">{product.name}</strong>? Its status will
                become <strong className="text-amber-700">INACTIVE</strong>. Historical invoices and
                transactions will remain intact, but it will be hidden from default listings.
              </>
            ) : (
              <>
                Are you sure you want to reactivate{' '}
                <strong className="text-gray-900 font-semibold">{product.name}</strong>? Its status will
                change back to <strong className="text-green-700">ACTIVE</strong> and become available for
                pricing and orders.
              </>
            )}
          </p>
