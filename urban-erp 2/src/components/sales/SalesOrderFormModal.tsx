...
import { calculateOrderTotals } from '../../services/salesOrderService';
...
export const SalesOrderFormModal: React.FC<SalesOrderFormModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
  initialData,
  ...
  const [items, setItems] = useState<ItemRowState[]>([
    { productId: '', productName: '', quantity: 1, unitPrice: 0, taxRate: 18 },
  ]);
  ...
  // Eligible customers: ONLY type === 'CUSTOMER' or 'BOTH' and ACTIVE
  const eligibleCustomers = useMemo(() => {
    return contacts.filter(
      c => (c.type === 'CUSTOMER' || c.type === 'BOTH') && c.status === 'ACTIVE'
    );
  }, [contacts]);
  ...
  useEffect(() => {
    if (initialData && mode === 'edit') {
      ...
      if (initialData.items && initialData.items.length > 0) {
        ...
        setItems([{ productId: '', productName: '', quantity: 1, unitPrice: 0, taxRate: 18 }]);
      }
    } else {
      setCustomerId('');
  ...
  const handleProductChange = (index: number, selectedProductId: string) => {
    ...
    setItems(prev => {
      ...
      copy[index] = {
        ...
        unitPrice: selectedProd ? selectedProd.salesPrice : copy[index].unitPrice,
      };
      return copy;
    });
  };
  ...
  const validate = (): boolean => {
    ...
    if (!customerId) {
      errs.customerId = 'Customer is required. Please select a customer contact.';
    }
    ...
    if (items.length === 0) {
      ...
      items.forEach((item, idx) => {
        ...
        if (item.taxRate === '' || Number(item.taxRate) < 0) {
          errs[`item_${idx}_tax`] = `Item #${idx + 1}: Tax rate must be 0 or greater.`;
        }
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedCustomer = contacts.find(c => String(c.id) === String(customerId));
    const customerName = selectedCustomer ? selectedCustomer.name : 'Customer #' + customerId;
    const customerEmail = selectedCustomer?.email;
    const customerMobile = selectedCustomer?.mobile;

    const formattedItems: CreateSalesOrderItemDTO[] = items.map(item => ({
    ...
    try {
      ...
      await onSubmit(payload, { customerName, customerEmail, customerMobile });
      onClose();
    } catch (err: any) {
      setApiError(err?.message || 'Failed to save sales order. Please verify backend connection.');
    } finally {
      setSubmitting(false);
    }
  };
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">
                {mode === 'create' ? 'Create Sales Order' : `Edit Sales Order (${initialData?.orderNumber || ''})`}
              </h2>
              <p className="text-xs text-gray-500">
                {mode === 'create'
                  ? 'Select customer, add product lines, and verify order totals'
                  : 'Update line items and order specifications before invoice generation'}
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto py-4 space-y-5 flex-1 pr-1">
          ...
          {!isEditable && (
            <div className="p-3.5 bg-amber-50 border border-amber-200 text-amber-900 rounded-xl text-xs flex items-start gap-2.5">
              ...
              <div>
                ...
                This sales order has status <span className="font-bold">{initialData?.status}</span>. Orders that are invoiced or cancelled cannot be modified to prevent corrupting historical records.
              </div>
            </div>
          )}

          {/* Top Section: Customer & Date Selection */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-700 uppercase tracking-wider mb-1.5 flex items-center justify-between">
                ...
                <span className="text-[11px] text-gray-400 font-normal">From Contacts</span>
              </label>

              {loadingData ? (
                <div className="flex items-center gap-2 text-xs text-gray-400 py-2">
                  <Loader2 className="w-4 h-4 animate-spin text-[#6D54B5]" />
                  <span>Loading customer contacts...</span>
                </div>
              ) : (
                <select
                  disabled={!isEditable}
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-gray-50/80 border-b border-gray-200 text-[11px] font-bold text-gray-500 uppercase tracking-wider">
                      <th className="py-2.5 px-3 w-4/12">Product</th>
                      <th className="py-2.5 px-3 w-2/12">Quantity</th>
                      <th className="py-2.5 px-3 w-2/12">Unit Price</th>
                      <th className="py-2.5 px-3 w-2/12">Tax (%)</th>
                      <th className="py-2.5 px-3 w-2/12 text-right">Line Total</th>
                      {isEditable && <th className="py-2.5 px-2 w-1/12 text-center"></th>}
                    </tr>
                  </thead>
                        ...
                        <tr key={idx} className="hover:bg-purple-50/10">
                            ...
                            >
                              ...
                              {activeProducts.map((p) => (
                                <option key={p.id} value={p.id}>
                                  {p.name} ({p.type}) • ₹{p.salesPrice.toLocaleString()}
                                </option>
                              ))}
                            </select>
                          ...
                          <td className="p-2.5">
                            <div className="relative">
                              <input
                                ...
                                className={`w-full pr-6 pl-2.5 py-1.5 rounded-lg border text-xs text-gray-900 focus:ring-1 focus:ring-[#6D54B5] ${
                                  taxErr ? 'border-red-300 bg-red-50/30' : 'border-gray-200'
                                }`}
                              />
                              <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 text-[11px]">
                                %
                              </span>
                            </div>
                            {taxErr && <p className="text-[10px] text-red-600 mt-0.5">{taxErr}</p>}
                          </td>
                          ...
                          <td className="p-2.5 text-right font-semibold text-gray-900">
                            ...
                            <div className="text-[10px] text-gray-400 font-normal">
                              (Tax: ₹{taxVal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })})
                            </div>
                          </td>
          ...
          <div className="bg-gray-50/80 rounded-2xl p-4 border border-gray-200/80 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="space-y-1 max-w-sm">
              ...
              <p className="text-[11px] text-gray-400 leading-relaxed">
                Subtotal = (Qty × Unit Price). Tax = (Subtotal × Tax Rate / 100). Final calculation verified and committed by Spring Boot.
              </p>
            </div>
            ...
            <div className="w-full sm:w-64 space-y-2 text-xs">
              ...
              <div className="flex justify-between text-gray-600">
                ...
                <span className="font-semibold text-gray-800">
                  ₹{calculatedTotals.taxAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="e.g. Standard delivery within 3 business days, customer purchase order reference"
          ...
          {isEditable && (
            ...
            >
              {submitting ? (
                ...
                <span>{mode === 'create' ? 'Save Sales Order' : 'Update Sales Order'}</span>
              )}
            </button>
          )}
        </div>
