...
import { Journal, JournalStatus } from '../../types/journal';

interface JournalDeactivateModalProps {
  isOpen: boolean;
  onClose: () => void;
  journal: Journal | null;
  onConfirm: (journal: Journal, newStatus: JournalStatus) => Promise<void>;
}

export const JournalDeactivateModal: React.FC<JournalDeactivateModalProps> = ({
  isOpen,
  onClose,
  journal,
  onConfirm,
  ...
  const isCurrentlyActive = journal.status === 'ACTIVE';
  const targetStatus: JournalStatus = isCurrentlyActive ? 'INACTIVE' : 'ACTIVE';

  const handleConfirm = async () => {
    setLoading(true);
    setApiError(null);
    try {
      await onConfirm(journal, targetStatus);
      onClose();
    } catch (err: any) {
      setApiError(err?.message || 'Failed to update journal status. Please try again.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="mt-2 space-y-3">
          <h3 className="text-base font-bold text-gray-900">
            {isCurrentlyActive ? 'Deactivate Journal?' : 'Reactivate Journal?'}
          </h3>
          <p className="text-xs text-gray-600 leading-relaxed">
            {isCurrentlyActive ? (
              <>
                Are you sure you want to deactivate{' '}
                <strong className="text-gray-900 font-semibold">{journal.name}</strong>? Its status will become{' '}
                <strong className="text-amber-700">INACTIVE</strong>. New transactions will not be assigned to this journal, but all existing journal entries remain preserved.
              </>
            ) : (
              <>
                Are you sure you want to reactivate{' '}
                <strong className="text-gray-900 font-semibold">{journal.name}</strong>? Its status will become{' '}
                <strong className="text-green-700">ACTIVE</strong> and will be available for ledger postings.
              </>
            )}
          </p>
          ...
          <div className="p-3 rounded-xl bg-blue-50/70 border border-blue-100 text-xs text-blue-900 flex items-start gap-2.5">
            ...
            <div className="leading-tight">
              ...
              Journals are never hard-deleted. Historical accounting entries, vouchers, and ledger history remain intact.
            </div>
          </div>
